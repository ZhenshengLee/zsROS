...
protected:
    ...
    //zs: add frame_id for world, cyton and mesa
...
void RosAriaNode::dynamic_reconfigureCB(rosaria::RosAriaConfig &config, uint32_t level)
{
  ...
  if(zsstart_pose_x != config.zsstart_pose_x)
  {
    ...
    ROS_INFO("Setting zsstart_pose_x from Dynamic Reconfigure: %lf", zsstart_pose_x);
  }
  if(zsstart_pose_y != config.zsstart_pose_y)
  {
    zsstart_pose_y = config.zsstart_pose_y;
    ROS_INFO("Setting zsstart_pose_y from Dynamic Reconfigure: %lf", zsstart_pose_y);
  }
  if(zsstart_pose_th != config.zsstart_pose_th)
  {
    zsstart_pose_th = config.zsstart_pose_th;
    ROS_INFO("Setting zsstart_pose_th from Dynamic Reconfigure: %lf", zsstart_pose_th);
  }
  //
  // Odometry Settings
  //
