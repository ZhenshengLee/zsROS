...
#include <rosaria/RosAriaConfig.h>  //a generated header <${package_name}>/<${cfg_name}.h>
...
class RosAriaNode
{
  ...
  protected:
    ...
    //zs: for more connector like cyton arm and bumblebee  connector
    ...
    //zs: add frame_id for world, cyton and bumblebee
    std::string tf_prefix;
    std::string frame_id_odom;
    std::string frame_id_base_link;
    std::string frame_id_bumper;
    std::string frame_id_sonar;
    std::string frame_id_zsworld;
    std::string frame_id_cyton;
    std::string frame_id_bumblebee;
    ...
    // zs: define the initial point of the robot
    // zs: by using a virtual global frame called "zsworld" and defining the transform between zsworld and odom
    // zs: use start_pos.setpose
  ...
  zsstart_pose_th(0.0)
{
  ...
  // port and baud
  n.param( "port", serial_port, std::string("/dev/ttyS0") );//zs:/dev/ttyUSB0
  ...
  // zs: tf_prefix is deprecated in newer ROS versions and in this project it will be always be ""
  // zs: add frame_id for world
  ...
  frame_id_zsworld = tf::resolve(tf_prefix, "zsworld_frame");
...
{
  //ROS_INFO_STREAM("zs: "<< publish_sonar << publish_sonar_pointcloud2 << publish_aria_lasers);
  ...
  zsworld_trans.transform.translation.x = zsstart_pose_x;
  zsworld_trans.transform.translation.y = zsstart_pose_y;
  ...
  if (publish_sonar || publish_sonar_pointcloud2)
  {
    ...
    //cloud.child_frame_id = frame_id_base_link;
    //cloud2.child_frame_id = frame_id_base_link;
...
int main( int argc, char** argv )
{
  ...
  //zs: TODO: add /zsworld frame and publish the transform with base_link
  //zs: TODO: may be in Setup function or the aria class constructor function
  if( node->Setup() != 0 )
  {
